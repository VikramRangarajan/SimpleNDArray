type cudaError_t = int
cudaMemcpyHostToHost: int = 0
cudaMemcpyHostToDevice: int = 1
cudaMemcpyDeviceToHost: int = 2
cudaMemcpyDeviceToDevice: int = 3

cudaSuccess: cudaError_t = 0


class dim3(list):
    x: int
    y: int
    z: int


class threadIdx(dim3): ...


class blockIdx(dim3): ...


class blockDim(dim3): ...


class gridDim(dim3): ...


warpSize: int
fprintf = printf = print


def __syncthreads(): ...
def atomicAdd[T](ptr: list[T], elem: T): ...
def atomicMin[T](ptr: list[T], elem: T): ...
def atomicMax[T](ptr: list[T], elem: T): ...


def cudaMalloc[T](ptr: list[list[T]], size: int) -> cudaError_t: ...
def cudaFree[T](ptr: list[T]) -> cudaError_t: ...
def cudaMemcpy(dst: list[None], src: list[None], size: int, mode: int) -> cudaError_t: ...
def static_assert(*args): ...
def cudaGetLastError() -> cudaError_t: ...
def cudaGetErrorString(err: cudaError_t) -> str: ...
def cudaGetErrorName(status: cudaError_t) -> str: ...
